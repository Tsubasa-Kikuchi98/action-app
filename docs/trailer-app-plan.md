...
| style | 尺 | 声 | 文字 | 使いどころ |
|---|---|---|---|---|
| `narration`（既定） | 約 29 秒 / 5 シーン | ナレ 3 本 ＋ セリフ 2 本（TTS） | テロップ・画面内テロップ・カード | 王道のハリウッド予告パロディ |
| `dialogue` | 約 30 秒 / 5 シーン | ナレ 2 本 ＋ セリフ 4 本（TTS） | 文字カード主導 | 邦画式。落差が大きい |
| **`nolan`**（2026-08-30 追加） | **20.0 秒 / 3 カット** | **ナレなし。セリフ 3 本を Veo が口パクで喋る** | **カードだけ**（カット内は文字ゼロ） | クリストファー・ノーラン作品の予告の形。静かなカメラ・鋼色・低音のブラーム |

#### nolan の構成（out/lambda-nolan で実測）
`IFTC 提供`(1.4s) → **先輩が気づく**(3s) → カード①＋ブラーム(1.6s) → **主人公がもがく**(4s) → カード②＋ブラーム(1.6s) → **上司が動き出す**(3s) → 無音の黒(0.4s) → タイトル(3s) → `近日公開`(2s)

- カードの文言は「気づいた時には、遅かった。」「時間は、待たない。」のような**時間・不可逆**の短い断言。事実の説明はしない
- カメラは据え置きか極遅ドリーだけ。手ブレ・疑似ズーム・白フラッシュ・xfade は使わない。1 クリップ 1 カット
- 色は彩度 0.80 の鋼色。teal-orange グレードとブルームは外す
- 音は Veo 同梱の環境音＋セリフが主役。カード開始と同時に ElevenLabs 製ブラーム、タイトルは riser
- 8 秒生成したクリップから**発話区間を音声解析で探して** 3〜4 秒を切り出す（頭から切るとセリフが切れる）
