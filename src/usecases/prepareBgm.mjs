...
async function fromElevenLabs(store, files, job) {
  const p = store.paths(job);
  ...
  files.write(out, Buffer.from(await res.arrayBuffer()));
  store.logEvent(job, {
    step: "bgm",
    ok: true,
...
export async function prepareBgm(deps, job, { force = false } = {}) {
  const { store, media, files, bgmDir } = deps;
  ...
  if (process.env.ELEVENLABS_API_KEY) {
    try {
      const out = await fromElevenLabs(store, files, job);
