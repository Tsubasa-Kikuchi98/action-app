...
//
// 手順:
//   1. シーン別レンダ  … img/sN.png + nar/sN.wav → scenes/sN.mp4（Ken Burns + drawtext）
...
import {
  ROOT, ensureDirs, readScript, jobPaths, ffmpeg, probeDuration,
...
async function renderScene(job, scene, i) {
  ...
  const fc = [
    ...
    `[0:v]scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},` +
      ...
      `setsar=1[bg]`,
    // テロップ（白・ドロップシャドウ・下部中央・フェードイン／アウト）
    ...
    // ナレーション（少し間を置いてから、シーン尺いっぱいまで無音で埋める）
    `[1:a]aresample=48000,aformat=sample_fmts=fltp:channel_layouts=stereo,` +
      `adelay=250|250,apad,atrim=0:${dur},asetpts=N/SR/TB[a]`,
  ].join(";\n");
...
export async function render(job, { force = false } = {}) {
  ...
  console.log(`[render] シーン別レンダ ${data.scenes.length}件`);
  const clips = [];
  for (const [i, scene] of data.scenes.entries()) {
    ...
    if (!force && fs.existsSync(out) && fs.statSync(out).size > 0) {
      ...
      const ts = Date.now();
      await renderScene(job, scene, i);
      console.log(`  s${i + 1}: ${scene.duration_sec}s (${((Date.now() - ts) / 1000).toFixed(1)}s)`);
    }
    clips.push(out);
  }
  ...
  logEvent(job, { step: "render", ok: true, sec: Number(sec.toFixed(2)), total_sec: Number(info.total.toFixed(2)), bgm: info.bgm ? path.basename(info.bgm) : null });
