  ...
  properties: {
    ...
    scenes: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["narration", "telop", "image_prompt", "duration_sec"],
        properties: {
          narration: {
            type: "string",
            description: "日本語のナレーション。1シーンあたり1文、20〜30字。低音の予告ナレーターがテンポよく読む前提。",
          },
          telop: {
            type: "string",
...
# 構成（全${SCENE_COUNT}シーン・合計30〜40秒）
1. 導入 —「その日、〇〇は__だった」のような静かな立ち上がり
2. 日常 — 平穏さ、あるいは何気ない前振り
3. 転機 —「しかし__」で始まる不穏な展開
4. 危機 — 最も追い詰められた瞬間。短い言葉で畳みかける
5. 決意・反撃 —「この夏」「今、__が動き出す」のような盛り上がり
6. 締め — タイトルコール用。作品タイトルを言い切る短い一言（「COMING SOON」は動画側で出すので書かない）

# ナレーション（narration）
- 日本語。1シーンにつき1文、20〜30字。体言止めと余韻を多用する。長い説明は禁止。
...
# 尺（duration_sec）
- 4〜7 秒。序盤はゆったり、中盤の畳みかけは短く、最後はやや長めにする。
- 合計が30〜40秒になるように配分する。
...
export async function generateScript(episode, job) {
  ...
  data.scenes = data.scenes.map((s, i) => ({
    ...
    duration_sec: Math.min(7, Math.max(4, Number(s.duration_sec) || 5)),
    index: i + 1,
  }));
